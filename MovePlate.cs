...
public class MovePlate : StaticObject
{
    ...
    public void OnMouseUp()
    {
        ...
        if (matrixX == 2 && matrixY == 0 && reference.name != "white_object")
        {
            Destroy(reference);
            controller.GetComponent<Game>().Winner("няшк");
        }
        if (matrixX == 2 && matrixY == 7 && reference.name != "black_object")
        {
            Destroy(reference);
            controller.GetComponent<Game>().Winner("комар");
        }
